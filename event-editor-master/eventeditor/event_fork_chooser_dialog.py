...
class EventForkChooserDialog(q.QDialog):
    ...
    def __init__(self, parent, flow_data: FlowData) -> None:
        ...
        self.setWindowTitle(tr('event.fork_chooser.title'))
        ...
        self.start_event_btn = q.QPushButton(tr('event.fork_chooser.select_event'))
        self.end_event_btn = q.QPushButton(tr('event.fork_chooser.select_event'))
        self.form.addRow(tr('event.fork_chooser.fork_before'), self.start_event_btn)
        self.form.addRow(tr('event.fork_chooser.join_after'), self.end_event_btn)
        self.connectEventButtons()
        self.initLayout()

    def initLayout(self) -> None:
        warning_msg = q.QLabel(tr('event.fork_chooser.warning'))
    ...
    def accept(self) -> None:
        if not self.start_event or not self.end_event:
            q.QMessageBox.critical(self, tr('event.fork_chooser.title'), tr('event.fork_chooser.no_selection'))
            return
        super().accept()

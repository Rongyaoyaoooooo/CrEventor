...
class ActorRelatedEventEditDialog(q.QDialog):
    def __init__(self, parent, flow_data: FlowData, idx: int, attr_list_name: str, attr_name: str) -> None:
        ...
        self.setWindowTitle(tr('event.edit.title'))
        ...
        if self.is_switch:
            self.setWindowTitle(tr('event.edit.switch_title'))
        else:
            self.setWindowTitle(tr('event.edit.action_title'))
        ...
        separator = q.QLabel(tr('event.edit.separator'))
    ...
    def onAutofillRequested(self) -> None:
        ...
        if not new_actor or not new_attr:
            q.QMessageBox.critical(self, tr('event.edit.cannot_autofill'), tr('event.edit.select_actor_function'))
            return

        aiprog = ai.load_aiprog(new_actor.identifier.name)
        if not aiprog:
            if not self.tryJsonAutofill(new_actor.identifier.name, new_attr):
                q.QMessageBox.critical(self, tr('event.edit.cannot_autofill'), tr('event.edit.ai_load_failed'))
            return
        ...
        if actual_ai_class is None:
            q.QMessageBox.critical(self, tr('event.edit.cannot_autofill'), tr('event.edit.not_in_ai'))
            return
    ...
    def onReorderRequested(self) -> None:
        ...
        new_attr: str = self.attr_cbox.currentData().v if self.attr_cbox.currentData() else ''
        if not new_actor or not new_attr:
            q.QMessageBox.critical(self, tr('event.edit.cannot_reorder'), tr('event.edit.select_actor_function'))
            return
        ...
        if parameters is None:
            q.QMessageBox.critical(self, tr('event.edit.cannot_reorder'), tr('event.edit.def_load_failed'))
            return
    ...
    def onPasteJsonRequested(self) -> None:
        try:
            ...
            q.QMessageBox.critical(self, tr('container.copy_json'), tr('event.edit.paste_failed'))
    ...
    def accept(self) -> None:
        ...
        if not new_actor or not new_attr:
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('event.edit.select_actor_function'))
            return
...
class SubFlowEventEditDialog(q.QDialog):
    def __init__(self, parent, flow_data: FlowData, idx: int) -> None:
        ...
        self.flowchart_ledit.setPlaceholderText(tr('event.edit.flowchart_placeholder'))
        form.addRow(tr('event.edit.flowchart_label'), self.flowchart_ledit)
        self.entry_point_ledit = q.QLineEdit()
        self.entry_point_ledit.setText(self.event.data.entry_point_name)
        self.entry_point_ledit.setPlaceholderText(tr('event.edit.entry_point_placeholder'))
        form.addRow(tr('event.edit.entry_point_label'), self.entry_point_ledit)
    ...
    def accept(self) -> None:
        ...
        if not new_ep:
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('event.edit.entry_point_empty'))
            return
...
def show_event_editor(parent, flow_data: FlowData, idx: int) -> bool:
    ...
    q.QMessageBox.information(parent, tr('event.edit.title'), tr('event.edit.no_editable'))
    return False

...
_PLACEHOLDER_EVENT.name = tr('event.branch_editor.placeholder')
...
class SwitchCaseModel(qc.QAbstractTableModel):
    ...
    def appendCase(self, case: SwitchCase) -> bool:
        if self.hasCaseValue(case.value):
            q.QMessageBox.critical(None, tr('container.invalid_data'), tr('event.branch_editor.duplicate_case_value', value=case.value))
            return False
    ...
    def setData(self, index: qc.QModelIndex, value, role) -> bool:
        ...
        if col == SwitchCaseModelColumn.Value and isinstance(value, int):
            if self.hasCaseValue(value):
                q.QMessageBox.critical(None, tr('container.invalid_data'), tr('event.branch_editor.duplicate_case_value', value=value))
                return False
        ...
        if role == qc.Qt.DisplayRole or role == qc.Qt.ToolTipRole or role == qc.Qt.EditRole:
            ...
            if col == SwitchCaseModelColumn.Event:
                ...
                if event is _PLACEHOLDER_EVENT:
                    return tr('event.branch_editor.placeholder')
    ...
    def headerData(self, section, orientation, role) -> qc.QVariant:
        ...
        if section == SwitchCaseModelColumn.Value:
            return tr('event.branch_editor.value')
        if section == SwitchCaseModelColumn.Event:
            return tr('event.branch_editor.event')
        return tr('event.branch_editor.unknown')
...
class SwitchEventEditDialog(q.QDialog):
    ...
    def __init__(self, parent, cases: _Cases, flow_data: FlowData) -> None:
        super().__init__(parent)
        self.setWindowTitle(tr('event.branch_editor.switch_title'))
        ...
        add_btn = q.QPushButton(tr('event.branch_editor.add_case'))
    ...
    def accept(self) -> None:
        if not self.model.isValid():
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('event.branch_editor.no_placeholder_cases'))
            return
    ...
    def addCase(self) -> None:
        value, ok = q.QInputDialog.getInt(self, tr('event.branch_editor.add_case_title'), tr('event.branch_editor.case_value_prompt'))
        if not ok:
            return
    ...
    def onContextMenu(self, pos) -> None:
        ...
        menu = q.QMenu()
        menu.addAction(tr('actor.context.remove'), lambda: self.model.removeCase(sidx.row()))
...
class ForkEventModel(qc.QAbstractListModel):
    ...
    def appendFork(self, fork: Event) -> bool:
        if self.hasFork(fork):
            q.QMessageBox.critical(None, tr('container.invalid_data'), tr('event.branch_editor.duplicate_fork', name=fork.name))
            return False
        self.beginInsertRows(qc.QModelIndex(), len(self.l), len(self.l))
        self.l.append(fork)
        self.endInsertRows()
...
class ForkEventEditDialog(q.QDialog):
    ...
    def __init__(self, parent, forks: _Forks, flow_data: FlowData) -> None:
        super().__init__(parent)
        self.setWindowTitle(tr('event.branch_editor.fork_title'))
        self.setMinimumWidth(600)
        self.flow_data = flow_data
        ...
        add_btn_box = q.QHBoxLayout()
        add_btn = q.QPushButton(tr('event.branch_editor.add_fork'))
    ...
    def accept(self) -> None:
        if not self.model.isValid():
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('event.branch_editor.no_placeholder_forks'))
            return

        if self.model.rowCount(qc.QModelIndex()) == 0:
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('event.branch_editor.fork_min_branch'))
            return

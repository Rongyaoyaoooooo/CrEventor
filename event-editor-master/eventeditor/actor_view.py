...
class ActorEditDialog(q.QDialog):
    def __init__(self, parent, flow_data, idx: int) -> None:
        ...
        self.setWindowTitle(tr('actor.edit_title'))
    ...
    def initWidgets(self, idx: int) -> None:
        ...
        self.form.addRow(tr('actor.name'), self.name_edit)
        self.mapper.addMapping(self.name_edit, ActorModelColumn.Name)

        self.sub_name_edit = q.QLineEdit()
        self.form.addRow(tr('actor.sub_name'), self.sub_name_edit)
    ...
    def createArgumentGroup(self) -> q.QGroupBox:
        ...
        group = q.QGroupBox(tr('actor.as_argument'))
    ...
    def accept(self) -> None:
        ...
        if identifier != self.prev_identifier and model.has(identifier):
            q.QMessageBox.critical(self, tr('actor.cannot_edit'), tr('actor.identifier_conflict', identifier=identifier))
            return
...
class ActorAddDialog(ActorEditDialog):
    def __init__(self, parent, flow_data, idx) -> None:
        ...
        self.setWindowTitle(tr('actor.add_title'))
...
class ActorDetailPane(q.QWidget):
    ...
    def connectWidgets(self) -> None:
        ...
        self.action_view.addActionBuilder(lambda menu, idx: menu.addAction(tr('actor.detail.jump_to_events'), lambda: self.onJumpToEvents(idx)))
        self.query_view.addActionBuilder(lambda menu, idx: menu.addAction(tr('actor.detail.jump_to_events'), lambda: self.onJumpToEvents(idx)))
        self.container_view.addActionBuilder(lambda menu, idx: menu.addAction(tr('actor.detail.add_default_params'), lambda: self.addDefaultCreateParameters()))
...
class ActorView(q.QWidget):
    ...
    def initWidgets(self) -> None:
        ...
        self.add_actor_btn = q.QPushButton(tr('actor.add_btn'))
    ...
    def connectWidgets(self) -> None:
        ...
        self._on_file_loaded_enable = lambda: self.add_actor_btn.setEnabled(True)
        self._on_file_loaded_hide = lambda: self.hideActorDetailPane()
        self.flow_data.fileLoaded.connect(self._on_file_loaded_enable)
        self.flow_data.fileLoaded.connect(self._on_file_loaded_hide)
    ...
    def _update_texts(self, _lang: str = '') -> None:
        ...
        self.add_actor_btn.setText(tr('actor.add_btn'))
        self.updateNumActorLabel()

    def setFlowData(self, new_fd) -> None:
        """Switch this view to a different FlowData (multi-flow workspace)."""
        if self.flow_data is new_fd:
            return
        # Disconnect old
        try:
            self.flow_data.fileLoaded.disconnect(self._on_file_loaded_enable)
        except TypeError:
            pass
        try:
            self.flow_data.fileLoaded.disconnect(self._on_file_loaded_hide)
        except TypeError:
            pass
        try:
            self.flow_data.flowDataChanged.disconnect(self.updateNumActorLabel)
        except TypeError:
            pass
        # Switch
        ...
        # Update views
        self.actor_view.setModel(self.flow_data.actor_model)
        self.detail_pane.flow_data = new_fd
    ...
    def updateNumActorLabel(self, *_) -> None:
        self.num_actors_label.setText(tr('actor.count', count=self.flow_data.actor_model.rowCount(None)))
    ...
    def removeActor(self, idx: qc.QModelIndex) -> None:
        ...
        if util.is_actor_in_use(self.flow_data.flow.flowchart.events, actor):
            q.QMessageBox.critical(self, tr('actor.cannot_remove'), tr('actor.cannot_remove_text', identifier=actor.identifier))
            return
    ...
    def onContextMenu(self, pos) -> None:
        ...
        menu = q.QMenu()
        menu.addAction(tr('actor.context.edit'), lambda: self.editActor(idx))
        menu.addAction(tr('actor.context.remove'), lambda: self.removeActor(idx))
        menu.addAction(tr('actor.detail.jump_to_events'), lambda: self.jumpToActorEventsRequested.emit(str(idx.data(qc.Qt.UserRole).identifier) + '::'))

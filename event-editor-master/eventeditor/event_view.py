...
class EventView(q.QWidget):
    ...
    def onContextMenu(self, pos) -> None:
        ...
        menu = q.QMenu()
        menu.addAction(tr('event.context.edit'), lambda: self.editEvent(source_idx.row()))
        menu.addAction(tr('event.context.jump_to_flowchart'), lambda: self.jumpToFlowchartRequested.emit(source_idx.row()))

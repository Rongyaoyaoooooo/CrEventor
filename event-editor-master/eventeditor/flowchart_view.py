...
class FlowchartWebObject(qc.QObject):
    ...
    dialogueTextsChanged = qc.pyqtSignal()
    ...
    def getData(self) -> typing.Union[list, dict]:
        flow = self.view.flow_data.flow
        graph = generate_flowchart_graph(flow) if flow else []
        # Only send dialogueTexts for MessageIds present in the current graph
        dialogue_texts = self._build_dialogue_texts_for_graph(graph)
        return {
            'graph': graph,
            'notesVisible': self.view.flow_data.get_notes_display(),
            'translations': self.view.flow_data.get_translations(),
            'dialogueTexts': dialogue_texts,
        }

    def _build_dialogue_texts_for_graph(self, graph: list) -> dict:
        """Build dialogueTexts dict for MessageIds in the current graph.

        Per spec: MessageId format is "EventFlowMsg/{name}:{key}" (no extension).
        MSYT path is "EventFlowMsg/{name}.msyt".  Lookup is exact path+key match.
        """
        texts: typing.Dict[str, str] = {}
        if not self.view.flow_data.text_database:
            return texts
        db = self.view.flow_data.text_database
        for entry in graph:
            if entry.get('type') != 'node':
                continue
            params = entry.get('data', {}).get('params')
            if not params:
                continue
            msg_id = params.get('MessageId')
            if not msg_id or not isinstance(msg_id, str) or ':' not in msg_id:
                continue
            text_entry = db.lookup_by_message_id(msg_id)
            if text_entry:
                texts[msg_id] = text_entry.text
        return texts
    ...
    @qc.pyqtSlot(int)
    def editDialogue(self, node_id: int):
        self.view.webEditDialogue(int(node_id))

    @qc.pyqtSlot(result=qc.QVariant)
    def getDialogueTexts(self) -> qc.QVariant:
        """Return all dialogue texts for the current graph (lightweight update)."""
        flow = self.view.flow_data.flow
        graph = generate_flowchart_graph(flow) if flow else []
        texts = self._build_dialogue_texts_for_graph(graph)
        return qc.QVariant(texts)
    ...
    @qc.pyqtSlot()
    def graphEmptySpaceContextMenu(self):
        self.view.graphEmptySpaceContextMenu.emit()
...
class FlowchartView(q.QWidget):
    ...
    graphEmptySpaceContextMenu = qc.pyqtSignal()
    ...
    def initWidgets(self) -> None:
        ...
        self.flow_data.notesDisplayChanged.connect(self.web_object.notesDisplayChanged)
        self.notesDisplayChanged.connect(self.web_object.notesDisplayChanged)

        self.flow_data.dialogueDataChanged.connect(self._onDialogueDataChanged)

        self.view = QWebEngineView()
        # Enable WebEngine remote debugging at http://localhost:9222
        ...
        self.view.loadFinished.connect(self._onPageLoaded)
        Tr.instance.languageChanged.connect(self.injectTranslations)
    ...
    def connectWidgets(self) -> None:
        ...
        self._fd_clear_sel = lambda reason: self.entry_point_view.clearSelection()
        self.flow_data.flowDataChanged.connect(self._fd_clear_sel)
        ...
        self._fd_refresh_params = lambda reason: self.refreshParamModel()
        self.flow_data.flowDataChanged.connect(self._fd_refresh_params)

        self.reloadedSignal.connect(self.onWebViewReloaded)

    def setFlowData(self, new_fd: FlowData) -> None:
        """Switch this view to a different FlowData (multi-flow workspace)."""
        if self.flow_data is new_fd:
            return
        # Disconnect old
        try:
            self.flow_data.flowDataChanged.disconnect(self.onFlowDataChanged)
        except TypeError:
            pass
        try:
            self.flow_data.flowDataChanged.disconnect(self._fd_clear_sel)
        except TypeError:
            pass
        try:
            self.flow_data.flowDataChanged.disconnect(self._fd_refresh_params)
        except TypeError:
            pass
        try:
            self.flow_data.fileLoaded.disconnect(self.web_object.fileLoaded)
        except TypeError:
            pass
        try:
            self.flow_data.notesDisplayChanged.disconnect(self.web_object.notesDisplayChanged)
        except TypeError:
            pass
        try:
            self.flow_data.dialogueDataChanged.disconnect(self._onDialogueDataChanged)
        except TypeError:
            pass
        # Set new
        self.flow_data = new_fd
        # Connect new
        self.flow_data.flowDataChanged.connect(self.onFlowDataChanged)
        self.flow_data.flowDataChanged.connect(self._fd_clear_sel)
        self.flow_data.flowDataChanged.connect(self._fd_refresh_params)
        ...
        self.flow_data.dialogueDataChanged.connect(self._onDialogueDataChanged)
        # Update views
        self.entry_point_view.selectionModel().clearSelection()
    ...
    def _onPageLoaded(self, ok: bool) -> None:
        if ok:
            self.injectTranslations()

    def injectTranslations(self) -> None:
        keys = [
            'edit_event', 'edit_cases', 'edit_branches',
            'add_entry_point', 'add_parent', 'add_child',
            'unlink_child', 'link_event', 'remove_event',
            'remove_entry_point', 'show_all', 'show_connected',
            'edit_dialogue', 'add_dialogue',
        ]
        tr_obj = {k: tr('flowchart.context.' + k) for k in keys}
        lang = Tr.instance.language
        js = 'window.TR = ' + json.dumps(tr_obj, ensure_ascii=False) + ';'
        js += 'window.LANG = ' + json.dumps(lang) + ';'
    ...
    def export(self) -> None:
        if not self.flow_data.flow:
            return
        path = q.QFileDialog.getSaveFileName(self, tr('flowchart.export.dialog_title'), self.flow_data.flow.name + '.json', 'Data (*.json)')[0]
        if not path:
            return
        ...
        try:
            ...
            q.QMessageBox.critical(self, tr('menu.flowchart.export_graph'), tr('flowchart.export.write_error', path=path))
    
    def export_definitions(self) -> None:
        try:
            aj.export_definitions(self.flow_data.flow, self)
        except:
            q.QMessageBox.critical(self, tr('flowchart.definitions.export_error'), tr('flowchart.definitions.export_error', path=str(aj._actor_definitions_path)))
    ...
    def webEditDialogue(self, idx: int) -> None:
        """Open the dialogue text editor for a Talk event node."""
        if idx < 0:
            return
        ...
        if text_db is None:
            q.QMessageBox.information(
                ...
                tr('dialogue.reload_failed'),
            )
            return

        message_id = _get_message_id(event)
        if message_id:
            # Look up via the same matching logic used for preview
            ...
            if entry:
                ...
                dialog = TextEditorIntegrationDialog(
                    self, text_db, entry.label, entry.msbt_file,
                )
                dialog.textSaved.connect(
                    lambda l, c: self.flow_data.dialogueDataChanged.emit())
                dialog.exec_()
                return
            # Not found in the correct MSYT file — offer to create
            ...
            default_msbt = ''
            if ':' in message_id:
                default_msbt = f'{message_id.split(":")[0]}.msbt'
            ...
            dialog.messageCreated.connect(
                lambda l, t: self._onMessageCreated())
            dialog.exec_()
            return
        else:
            # No MessageId parameter — offer to create one
            from CrEventor.message_creator import MessageCreatorDialog
            flow_name = self.flow_data.flow.name if self.flow_data.flow else 'Unknown'
            dialog = MessageCreatorDialog(
                self, text_db, flow_name, event, idx,
            )
            dialog.messageCreated.connect(
                lambda l, t: self._onMessageCreated())
            dialog.exec_()
    ...
    def _onMessageCreated(self) -> None:
        """Handle a newly created message — refresh data."""
        self.flow_data.flowDataChanged.emit(FlowDataChangeReason.EventParameters)
    ...
    def webAddEntryPoint(self, event_idx: int) -> None:
        ...
        ep_name, ok = q.QInputDialog.getText(self, tr('flowchart.add_entry_point'), ftr('flowchart.entry_point_name'), q.QLineEdit.Normal)
    ...
    def webRemoveEntryPoint(self, ep_idx: int) -> None:
        try:
            ...
            q.QMessageBox.critical(self, 'Bug', tr('flowchart.fork_error', error=e))
    ...
    def webAddEventAbove(self, parent_indices: typing.List[int], event_idx: int) -> None:
        ...
        if parent_events:
            ...
            dialog.setWindowTitle(tr('flowchart.add_above_title'))
            ...
            dialog_layout.addWidget(q.QLabel(tr('flowchart.add_above_hint')))
    ...
    def webDoLink(self, event: Event, target: Event) -> None:
        if event == target:
            q.QMessageBox.critical(self, tr('flowchart.invalid_selection'), tr('flowchart.cannot_self_link'))
            return
    ...
    def webUnlink(self, event_idx: int) -> None:
        ret = q.QMessageBox.question(self, tr('flowchart.unlink_warning'), tr('flowchart.unlink_warning_text'))
    ...
    def _doRemoveEvent(self, parents: typing.List[Event], event_idx: int) -> None:
        ...
        if len(parents) == 1 and isinstance(parents[0].data, ForkEvent) and len(parents[0].data.forks) == 1 and not next_event:
            q.QMessageBox.information(self, tr('flowchart.cannot_remove'), tr('flowchart.cannot_remove_text'))
            return
    ...
    def _doAddFork(self, start: Event, end: Event) -> None:
        if not (isinstance(end.data, ActionEvent) or isinstance(end.data, SubFlowEvent)):
            q.QMessageBox.critical(self, tr('flowchart.not_implemented'), tr('flowchart.must_be_action'))
            return
    ...
    def _onDialogueDataChanged(self) -> None:
        """Refresh only dialogue texts without full graph reload."""
        self.web_object.dialogueTextsChanged.emit()


# ---------------------------------------------------------------------------
# Helper functions for dialogue text matching
# ---------------------------------------------------------------------------

# Talk-related action names — nodes whose action name contains any of
# these tokens are treated as dialogue nodes.
_TALK_ACTION_KEYWORDS = frozenset({
    'Talk',
    'EventTalk',
    'DemoTalk',
    'NpcTalk',
...
def _is_talk_event(event: Event) -> bool:
    """Return True if this event is a Talk-type dialogue node."""
    data = event.data
    if isinstance(data, ActionEvent):
        action_name = str(data.actor_action.v) if data.actor_action.v else ''
        return any(kw in action_name for kw in _TALK_ACTION_KEYWORDS)
    return False


def _get_message_id(event: Event) -> typing.Optional[str]:
    """Extract the MessageId parameter from an event, if present."""
    data = event.data
    if not hasattr(data, 'params') or not data.params:
        return None
    msg_id = data.params.data.get('MessageId')
    return msg_id if msg_id else None


def _msg_id_to_msyt_path(message_id: str) -> typing.Optional[str]:
    """Convert a MessageId like 'EventFlowMsg/Actor:Talk_40' to
    the corresponding .msyt path like 'EventFlowMsg/Actor.msyt'."""
    if not message_id or ':' not in message_id:
        return None
    path_part = message_id.rsplit(':', 1)[0]

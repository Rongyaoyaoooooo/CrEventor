...
class ContainerAddItemDialog(q.QDialog):
    def __init__(self, parent, model: ContainerModel) -> None:
        ...
        self.setWindowTitle(tr('container.add_title'))
        self.resize(500, 100)
        self.model = model
        ...
        form_layout.addRow(tr('container.key'), self.key_ledit)
        form_layout.addRow(tr('container.type'), self.type_layout)
        form_layout.addRow(tr('container.value'), self.value_widget)
    ...
    def createTypeWidgets(self) -> None:
        ...
        add_type_choice(tr('container.type_value'), ContainerAddItemDialogType.Value)
        add_type_choice(tr('container.type_actor_id'), ContainerAddItemDialogType.ActorIdentifier)
        add_type_choice(tr('container.type_argument'), ContainerAddItemDialogType.Argument)
    ...
    def createValueWidgets(self) -> None:
        ...
        self.tedit.setPlaceholderText(tr('container.value_placeholder'))
        ...
        actor_id_form.addRow(tr('container.name'), self.actor_id_name_box)
        self.actor_id_sub_name_box = q.QLineEdit()
        actor_id_form.addRow(tr('container.sub_name'), self.actor_id_sub_name_box)
    ...
    def parseValue(self) -> typing.Any:
        try:
            ...
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.parse_failed', error=e))
            return None

        if isinstance(data, list):
            if not data:
                q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.array_empty'))
                return None
            if not util.is_valid_container_value_type(data[0]):
                q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.invalid_type', type_name=type(data[0]).__name__))
                return None
            if not util.is_list_homogeneous(data):
                q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.array_mixed'))
                return None
        else:
            if not util.is_valid_container_value_type(data):
                q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.invalid_type', type_name=type(data).__name__))
                return None
    ...
    def parseActorIdentifier(self) -> typing.Any:
        ...
        if not identifier.name:
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.actor_name_empty'))
            return None
        return identifier
    ...
    def parseArgument(self) -> typing.Any:
        ...
        if not argument:
            q.QMessageBox.critical(self, tr('container.invalid_data'), tr('container.argument_empty'))
            return None
        return argument
    ...
    def accept(self) -> None:
        ...
        if not key_name:
            q.QMessageBox.critical(self, tr('container.invalid_key'), tr('container.key_empty'))
            return
        if self.model.has(key_name):
            q.QMessageBox.critical(self, tr('container.invalid_key'), tr('container.key_exists', key=key_name))
            return
...
class ContainerView(q.QWidget):
    ...
    def _update_texts(self, _lang: str = '') -> None:
        ...
        self._params_label.setText(tr('container.parameters'))
        self.add_btn.setText(tr('container.add'))
        self.autofill_btn.setText(tr('container.auto_fill'))
        self.reorder_btn.setText(tr('container.reorder'))
        self.copy_btn.setText(tr('container.copy_json'))
        self.paste_btn.setText(tr('container.paste_json'))
    ...
    def onContextMenu(self, pos) -> None:
        ...
        menu = q.QMenu()
        menu.addAction(tr('container.convert_to_argument'), lambda: self.onConvertToArgument(idx))
        menu.addAction(tr('container.remove_item'), lambda: self.onRemove(idx))

...
class EventTypeChooserDialog(q.QDialog):
    def __init__(self, parent) -> None:
        ...
        self.setWindowTitle(tr('event.chooser_dialog.type_title'))

        self.rbtn_group = q.QButtonGroup()
        action_rbtn = q.QRadioButton(tr('event.type_action'))
        action_rbtn.setChecked(True)
        self.rbtn_group.addButton(action_rbtn, EventType.kAction)
        switch_rbtn = q.QRadioButton(tr('event.type_switch'))
        self.rbtn_group.addButton(switch_rbtn, EventType.kSwitch)
        subflow_rbtn = q.QRadioButton(tr('event.type_subflow'))
...
_PLACEHOLDER_ACTOR.identifier.name = tr('event.chooser_dialog.placeholder_actor')
_PLACEHOLDER_ACTOR.actions.append(StringHolder(tr('event.chooser_dialog.placeholder_action')))
_PLACEHOLDER_ACTOR.queries.append(StringHolder(tr('event.chooser_dialog.placeholder_query')))
...
def add_new_event(parent, flow_data: FlowData) -> typing.Optional[Event]:
    ...
    if etype == EventType.kAction:
        ...
        new_event.data.entry_point_name = tr('event.chooser_dialog.placeholder_subflow')

    global _placeholder_warning_shown
    if not _placeholder_warning_shown:
        q.QMessageBox.warning(parent, tr('event.chooser_dialog.save_warning'), tr('event.chooser_dialog.save_warning_text'))
...
class EventChooserDialog(q.QDialog):
    def __init__(self, parent, flow_data: FlowData, enable_ctx_menu: bool=True) -> None:
        super().__init__(parent, qc.Qt.WindowTitleHint | qc.Qt.WindowSystemMenuHint)
        self.setWindowTitle(tr('event.chooser.title'))
        ...
        add_event_btn = q.QPushButton(tr('event.chooser_dialog.add_event'))
    ...
    def accept(self) -> None:
        ...
        if not selected_event:
            q.QMessageBox.critical(self, tr('event.chooser_dialog.events_title'), tr('event.chooser.no_selection'))
            return
        ...
        if role == qc.Qt.DisplayRole or role == qc.Qt.ToolTipRole:
            ...
            if branch is not None:
                if isinstance(branch, int):
                    description += tr('event.chooser_dialog.case_prefix') + str(branch)
                else:
                    description += tr('event.chooser_dialog.branch_prefix') + str(branch.v.name)
            return description
        if role == qc.Qt.CheckStateRole:
...
class CheckableEventParentListWidget(q.QWidget):
    def __init__(self, parent, child_event: Event, events: typing.List[Event]) -> None:
        ...
        all_btn = q.QPushButton(tr('event.chooser_dialog.all'))
        all_btn.clicked.connect(lambda: self.model.selectAll())
        none_btn = q.QPushButton(tr('event.chooser_dialog.none'))
